...
function make_form($email=null, $username=null,  $birthdate=null, $address_street=null, $address_city=null){
...
//if Posted, check the fields if it meets the requirements
if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    // Test username requirement - change to false if fail
    ...
    // Test Password requirement - change to false if fail
    $passwordRequirementLowerCase = false;
    $passwordRequirementUpperCase = false;
    $passwordRequirementNumber = false;
    for($i = 0; $i< strlen($password); $i++){       
        if(ord($password[$i])>= ord('a') and ord($password[$i])<= ord('z') ){
            $passwordRequirementLowerCase = true;
        } 
        if(ord($password[$i])>= ord('A') and ord($password[$i])<= ord('Z') ){
            $passwordRequirementUpperCase = true;
        }
        if(ord($password[$i])>= ord('0') and ord($password[$i])<= ord('9') ){
            $passwordRequirementNumber = true;
        }
    }
    ...
    else if ($passwordRequirementLowerCase === false or $passwordRequirementUpperCase === false or $passwordRequirementNumber === false){
